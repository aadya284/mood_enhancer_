...
export default function Dashboard() {
  ...
  const getTotalRecommendations = () => {
    return moodHistory.reduce((total, entry) => {
      return (
        total +
        Object.values(entry.recommendations).reduce(
          (sum, count) => sum + count,
          0,
        )
      );
  ...
  const getMostUsedCategory = () => {
    ...
    const maxCategory = Object.entries(totals).reduce((a, b) =>
      totals[a[0] as keyof typeof totals] > totals[b[0] as keyof typeof totals]
        ? a
        : b,
    );
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-xl text-white/80">
            Track your emotional journey and discover insights
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <Card className="dark-card">
              ...
              <CardContent>
                ...
                <Progress
                  value={weeklyStats.averageMood * 10}
            ...
            <Card className="dark-card">
              ...
              <CardContent>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span
                    className={`text-2xl font-bold capitalize ${getMoodColor(weeklyStats.mostCommonMood)}`}
                  >
        ...
        <Card className="dark-card mb-8">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {(
                ["movies", "music", "podcasts", "audiobooks", "games"] as const
              ).map((category) => {
                const total = moodHistory.reduce(
                  (sum, entry) => sum + entry.recommendations[category],
                  0,
                );
                const percentage =
                  total > 0 ? (total / getTotalRecommendations()) * 100 : 0;
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`font-semibold capitalize ${getMoodColor(entry.mood)}`}
                      >
                        {entry.mood}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-1">
                      {Object.entries(entry.recommendations).map(
                        ([category, count]) =>
                          count > 0 && (
                            <Badge
                              key={category}
            ...
            {moodHistory.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-white/70 mb-4">
                  Check your mood to start tracking your emotional journey
                </p>
                <Link to="/assess">
                  <Button className="bg-white text-primary hover:bg-white/90">
                    <Plus className="w-4 h-4 mr-2" />
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 text-white/80">
                  ...
                  <span className="text-sm">
                    Music recommendations work best for you
                  </span>
                </div>
                <div className="flex items-center gap-2 text-white/80">
                  <div className="w-2 h-2 bg-primary rounded-full"></div>
                  <span className="text-sm">
                    Regular mood checks boost your awareness
                  </span>
                </div>
              </div>
            </CardContent>
