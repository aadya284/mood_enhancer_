...
export default function SignUp() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    toast({ title: "Account created (demo)", description: "Authentication not yet connected." });
    navigate("/dashboard");
  }
    ...
    <div className="min-h-screen bg-black text-white grid place-items-center px-4">
      <Card className="w-full max-w-md bg-zinc-900 border border-white/10">
        <CardHeader>
          ...
          <CardDescription className="text-white/70">Join Moody to track and improve your mood.</CardDescription>
        </CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={onSubmit}>
            <div className="space-y-2">
              <Label htmlFor="name" className="text-white">Name</Label>
              <Input id="name" type="text" placeholder="Your name" required className="bg-black text-white placeholder:text-white/40" />
            </div>
            <div className="space-y-2">
            ...
            <Button type="submit" className="w-full bg-white text-black hover:bg-white/90">Create account</Button>
          </form>
